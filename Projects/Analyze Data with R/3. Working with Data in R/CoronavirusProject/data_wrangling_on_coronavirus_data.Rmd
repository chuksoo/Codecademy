...
setwd('C:/Users/hotty/Documents/Codecademy Dev/Codecademy/Projects/Analyze Data with R/3. Working with Data in R/CoronavirusProject')

# Load the data
covid_confirmed <- read.csv('corona_confirmed.csv')
covid_deaths <- read.csv('corona_deaths.csv')
covid_recovered <- read.csv('corona_recovered.csv')
...
total_cases <- covid_confirmed %>% 
  ...
  select(`3/22/20`) %>%
  sum()
print(paste('There were a total of', total_cases, 'cases in countries on March 22nd that are north of the equator'))

# Filter for Australia cases
cases_australia <- covid_confirmed %>%
  filter(`Country/Region` == 'Australia') %>%
  select(`3/22/20`) %>%
  sum()
print(paste('There were a total of', cases_australia, 'confirmed cases in Australia on March 22nd'))

# Display percentages
total_world_cases <- covid_confirmed %>%
  select(`3/22/20`) %>%
  sum()
print(paste('There were a total of', total_world_cases, 'in the world on March 22nd'))

# percentage of cases in northern hemisphere to the world
percentage_northern_hemisphere <- total_cases / total_world_cases * 100 
print(paste('The cases in northern hemisphere represents', round(percentage_northern_hemisphere, 2), '% of cases in the world'))

# percentage of cases in Australia to the world
percentage_australia <- cases_australia / total_world_cases * 100 
print(paste('The cases in Australia represents', round(percentage_australia, 2), '% of cases in the world'))
```
...
```{r}
# Group by countries
group_countries <- covid_confirmed %>%
...
# Total world recovered cases
total_world_recovered_cases <- covid_recovered %>%
  select(`3/22/20`) %>%
  sum()
#total_world_recovered_cases

# Total US recovered
group_us_percentage <- covid_recovered %>%
  group_by(`Country/Region`) %>%
  summarize_at(vars(-Lat, -Long, -`Province/State`), sum) %>%
...
# Display percentages
percentage_recovered_us = group_us_percentage / total_world_recovered_cases * 100
print(paste('There are', percentage_recovered_us, '% of recovered cases in the US'))
```
...
recovered_cases_us <- covid_recovered %>%
  group_by(`Country/Region`) %>%
  summarize_at(vars(-Lat, -Long, -`Province/State`), sum) %>%
  filter(`Country/Region` == 'US')
recovered_cases_us
...
max_recovered <- recovered_cases_us %>%
  select(-`Country/Region`) %>%
  max()

max_confirmed_cases_us <- covid_confirmed %>%
  group_by(`Country/Region`) %>%
  summarize_at(vars(-Lat, -Long, -`Province/State`), sum) %>%
  filter(`Country/Region` == 'US') %>%
  select(-`Country/Region`) %>%
...
max_recovered / max_confirmed_cases_us * 100
```
...
transposed_confirmed <- group_countries %>%
  t() %>%
  as.data.frame()
...
header.true(transposed_confirmed)
```
...
max_cases_us <- transposed_confirmed %>%
  select(US) %>%
  max()
max_cases_us
```
...
transposed_confirmed %>% 
  ggplot(aes(x = date, y = Italy)) + 
  geom_line() +
  labs(x = "Number of days since January 22nd, 2020", 
       y = "Number of confirmed cases", 
       title = "Confirmed cases of COVID-19 in Italy")
```
